...
* **Template Patch Matching**: Extracting data points from a scatter plot? Draw a bounding box around any data point (or just click it to auto-detect its bounds), and hit "Find Similar Points". Graphitizer's high-speed Normalized Cross-Correlation (NCC) engine will instantly map every matching marker with sub-pixel accuracy!
