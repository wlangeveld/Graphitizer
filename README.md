...
* **Template Patch Matching**: Extracting data points from a scatter plot? You can define a custom matching template around any data point (or just click it to auto-detect its bounds), and hit "Find Similar Points". Graphitizer's high-speed Normalized Cross-Correlation (NCC) engine will instantly map every matching marker with sub-pixel accuracy! (See the built-in Help dialog for detailed instructions on using the magnifier to draw patch templates).
